...
namespace VRSF.Multiplayer
{
    ...
    public class VRSFRemoteControllersInstantiater : MonoBehaviourPun
    {
        private void Start()
        {
        ...
        private void InstantiateRemoteControllers()
        {
            ...
            {
                Debug.LogFormat("<b>[VRSF] :</b> No mesh found for the Remote Player " + photonView.Owner.NickName + " with VR device " + deviceUsed + " in the ControllerMeshLister list.", gameObject);
            }
        }
    }
...
#if UNITY_EDITOR
    [UnityEditor.CustomEditor(typeof(VRSFRemoteControllersInstantiater))]
    public class VRSFRemoteControllersInstantiaterEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            UnityEditor.EditorGUILayout.HelpBox("For Local Player, please use a basic ControllerMeshAuthoring component by placing it under your SetupVR prefab.\n" +
                "A prefab is available by Right Clicking in the Hierarchy > VRSF > Controllers > Left/Right Controller", UnityEditor.MessageType.Info);
        }
    }
#endif
}