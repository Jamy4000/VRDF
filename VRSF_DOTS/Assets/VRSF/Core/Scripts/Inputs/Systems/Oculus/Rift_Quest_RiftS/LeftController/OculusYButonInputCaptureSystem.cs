    ...
    public class OculusYButonInputCaptureSystem : JobComponentSystem
    {
        ...
        struct YButtonInputCaptureJob : IJobForEachWithEntity<BaseInputCapture>
        {
            ...
            public void Execute(Entity entity, int index, ref BaseInputCapture baseInput)
            {
                ...
                if (YClickButtonDown)
                {
                    Commands.AddComponent(index, entity, new StartClickingEventComp { HasWaitedOneFrameBeforeRemoval = false, ButtonInteracting = EControllersButton.Y_BUTTON });
                    baseInput.IsClicking = true;
                    baseInput.IsTouching = false;
                }
                else if (YClickButtonUp)
                {
                    Commands.AddComponent(index, entity, new StopClickingEventComp { HasWaitedOneFrameBeforeRemoval = false, ButtonInteracting = EControllersButton.Y_BUTTON });
                ...
                {
                    Commands.AddComponent(index, entity, new StartTouchingEventComp { HasWaitedOneFrameBeforeRemoval = false, ButtonInteracting = EControllersButton.Y_BUTTON });
                    baseInput.IsTouching = true;
                }
                else if (baseInput.IsTouching && YTouchButtonUp)
                {
                    Commands.AddComponent(index, entity, new StopTouchingEventComp { HasWaitedOneFrameBeforeRemoval = false, ButtonInteracting = EControllersButton.Y_BUTTON });
                    baseInput.IsTouching = false;
                }
            }
        }
