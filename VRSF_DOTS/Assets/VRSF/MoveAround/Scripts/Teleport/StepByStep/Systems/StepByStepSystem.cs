...
namespace VRSF.MoveAround.Teleport
{
    ...
    public class StepByStepSystem : ComponentSystem
    {
        protected override void OnUpdate()
        {
            ...
            Entities.ForEach((ref StepByStepComponent sbs, ref GeneralTeleportParameters gtp, ref TeleportNavMesh tnm, ref VRRaycastOutputs raycastOutputs) =>
            {
                switch (gtp.CurrentTeleportState)
                {
                    case ETeleportState.Teleporting:
                        canTeleport = UserIsOnNavMesh(gtp, sbs, tnm, raycastOutputs, out Vector3 newUsersPos);
                        newPos = newUsersPos;
        ...
        private bool UserIsOnNavMesh(GeneralTeleportParameters gtp, StepByStepComponent sbs, TeleportNavMesh tnm, VRRaycastOutputs raycastOutputs, out Vector3 newCameraRigPos)
        {
            ...
            if (Physics.Raycast(vrCamTransform.position, directionVector, out RaycastHit hit, distanceVector, ~gtp.ExcludedLayers))
