...
namespace VRDF.Core.LaserPointer
{
    ...
    public class LaserPointerLengthSystem : ComponentSystem
    {
            ...
            Entities.ForEach((ref LaserPointerLength laserLength, ref VRRaycastOrigin raycastOrigin, ref VRRaycastOutputs raycastOutputs) =>
            {
                ...
                switch (raycastOrigin.RayOrigin)
                {
                    case ERayOrigin.LEFT_HAND:
                        if (!FetchTransformOrigin(VRDF_Components.LeftController, out originTransform))
                            return;
                        break;
                    case ERayOrigin.RIGHT_HAND:
                        if (!FetchTransformOrigin(VRDF_Components.RightController, out originTransform))
                            return;
                        break;
                    case ERayOrigin.CAMERA:
                        if (!FetchTransformOrigin(VRDF_Components.VRCamera, out originTransform))
                            return;
                        break;
                    default:
        ...
        private bool FetchTransformOrigin(GameObject toFetchFrom, out Transform originTransform)
        {
            if (toFetchFrom == null)
            {
                Debug.LogError("<b>[VRDF] :</b> The transform origin of the Laser couldn't be found. Skipping to next frame.");
